"""Web automation agent for LangOrch with Playwright and CDP support.
...
class AgentSettings:
    ...
    agent_id: str = "web-automation-agent"
    agent_name: str = "Web Automation Agent"
    agent_port: int = 9000
    channel: str = "web"
...
SETTINGS = AgentSettings(
    ...
    agent_id=os.getenv("WEB_AGENT_ID", "web-automation-agent"),
    agent_name=os.getenv("WEB_AGENT_NAME", "Web Automation Agent"),
...
app = FastAPI(title="LangOrch Web Automation Agent", version="0.2.0", lifespan=lifespan)
...
async def capabilities() -> dict[str, Any]:
    ...
    return {
        ...
        "description": "Playwright-based web automation agent for production workflows",
    }
...
async def _close_web_runtime(session_key: str) -> None:
    page = _pages.pop(session_key, None)
    context = _contexts.pop(session_key, None)
    _all_pages.pop(session_key, None)
    _active_frames.pop(session_key, None)
    _cdp_sessions.pop(session_key, None)
...
_AUTOMATION_SETTINGS = AutomationAgentSettings(
...
_COMMON_INPUT_PROPERTIES = {
    "session_id": string_schema(),
    "url": string_schema(),
    "target": string_schema(),
    "target_re": string_schema(),
    "text": string_schema(),
    "value": string_schema(),
    "key": string_schema(),
    "keys": {"oneOf": [string_schema(), array_schema(string_schema())]},
    "attribute": string_schema(),
    "expression": string_schema(),
    "js": string_schema(),
    "selector": string_schema(),
    "scope": string_schema(),
    "description": string_schema(),
    "element_description": string_schema(),
    "method": string_schema(),
    "timeout_ms": {"type": "integer"},
    "timeout_s": {"type": "number"},
}

_BASE_OUTPUT_SCHEMA = object_schema(
    required=["ok", "action"],
    properties={"ok": {"type": "boolean"}, "action": string_schema()},
)
...
_REQUIRED_FIELDS: dict[str, list[str]] = {
    ...
    "press_key": ["key"],
    "get_attribute": ["target", "attribute"],
    "extract_text": ["target"],
    "extract_table_data": ["target"],
    "select_all_text": ["target"],
...
_HIGH_RISK_ACTIONS = {"upload_file", "intercept_requests"}
...
def _protocol_capability(item: dict[str, Any]) -> AgentCapability:
    name = str(item["name"])
    if name in _HIGH_RISK_ACTIONS:
        side_effect_level = "high"
    elif name in _MEDIUM_RISK_ACTIONS:
    ...
    return AgentCapability(
        name=name,
        type="tool",
        description=f"Web automation action: {name}",
        input_schema=object_schema(required=_REQUIRED_FIELDS.get(name, []), properties=_COMMON_INPUT_PROPERTIES),
        output_schema=_BASE_OUTPUT_SCHEMA,
        side_effect_level=side_effect_level,
        idempotent=side_effect_level == "none",
        requires_approval=name in _HIGH_RISK_ACTIONS,
        session_scoped=True,
    )


PROTOCOL_CAPABILITIES: list[AgentCapability] = [
    AgentCapability(
        name="open_session",
        description="Create a reusable browser session",
        input_schema=object_schema(
            properties={
                "start_url": string_schema(),
                "viewport": object_schema(properties={"width": {"type": "integer"}, "height": {"type": "integer"}}),
                "storage_state": object_schema(),
                "extra_headers": object_schema(),
            }
        ),
    ...
    *[_protocol_capability(item) for item in CAPABILITIES],
]

SESSION_STORE = build_session_store(agent_id=_AUTOMATION_SETTINGS.agent_id, channel=_AUTOMATION_SETTINGS.channel, timeout_s=_AUTOMATION_SETTINGS.session_timeout_s)


async def _execute_protocol(action: str, params: dict[str, Any], req: AgentExecuteRequest) -> dict[str, Any]:
    session_key = str(req.session_id or params.get("session_id") or req.run_id)
    if SETTINGS.dry_run:
        return await _execute_dry_run(action, params)
    return await _execute_playwright(action, params, session_key)


app = build_agent_app(
    title="LangOrch Web Automation Agent",
    version="0.3.0",
    settings=_AUTOMATION_SETTINGS,
    capabilities=PROTOCOL_CAPABILITIES,
    execute_handler=_execute_protocol,
    health_provider=lambda: {"mode": "dry_run" if SETTINGS.dry_run else "playwright"},
    session_store=SESSION_STORE,
    session_close_handler=_close_web_runtime,
)
