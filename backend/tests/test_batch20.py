...
class TestHMACVerification:
    ...
    def test_no_secret_configured_fails_closed(self):
        ...
        assert not verify_hmac_signature(b"anything", None, "NONEXISTENT_SECRET_VAR")
...
class TestTriggerConcurrencyGuards:
    ...
    async def test_receive_webhook_locks_before_dedupe_check(self):
        ...
        async def fake_get_latest(_db, _procedure_id, trigger_type=None, enabled_only=True):
            return reg
        ...
        async def fake_fire(**_kwargs):
            order.append("fire")
            return {"entity_type": "run", "run_id": run.run_id}

        async def fake_record(_db, _procedure_id, _run_id, _payload_hash):
            order.append("record")

        with (
            patch("app.api.triggers.trigger_service.get_latest_trigger_for_procedure", new=fake_get_latest),
