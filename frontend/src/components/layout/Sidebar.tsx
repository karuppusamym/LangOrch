...
const NAV_SECTIONS: { title?: string; items: { href: string; label: string; d: string; d2?: string; badge?: "failed" | "pending" }[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/builder",    label: "Builder",    d: "M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" },
            ...
            {section.items.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center gap-3 mx-2 px-4 py-2 rounded-lg text-sm transition-colors ${
                    isActive
        ...
        >
          ...
          <div className="flex-1 min-w-0 text-left">
            <div className="text-sm font-medium text-neutral-900 dark:text-neutral-100 truncate">
              {user?.full_name ?? user?.identity ?? "User"}
            </div>
            <div className={`text-xs font-medium px-1.5 py-0.5 rounded-full inline-block mt-0.5 ${roleColor}`}>
              {user?.role ?? "viewer"}
            </div>
          </div>
