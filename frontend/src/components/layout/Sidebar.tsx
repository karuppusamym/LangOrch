...
const NAV_SECTIONS: { title?: string; items: { href: string; label: string; d: string; d2?: string; badge?: "failed" | "pending" }[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/builder", label: "Visual Builder", d: "M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" },
      ...
      { href: "/batch-jobs", label: "Batch Jobs", d: "M4 7h16M4 12h16M4 17h10" },
...
export default function Sidebar({
  ...
  }, [pathname, effectiveCollapsed, isMobile, isOpen]);
  ...
  return (
    <aside
      className={`fixed inset-y-0 left-0 z-30 flex flex-col border-r border-neutral-200 bg-white transition-[width,transform] duration-200 dark:border-neutral-800 dark:bg-neutral-900 ${isMobile ? `w-72 ${isOpen ? "translate-x-0" : "-translate-x-full"}` : effectiveCollapsed ? "w-20" : "w-64"}`}
    >
      {/* Logo */}
      <div className={`flex h-[var(--header-height)] shrink-0 items-center border-b border-neutral-200 dark:border-neutral-800 ${effectiveCollapsed ? "justify-center px-2" : "gap-2 px-6"}`}>
        <svg className="w-6 h-6 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
        </svg>
        {!effectiveCollapsed && <span className="font-bold text-lg text-neutral-900 dark:text-neutral-100">LangOrch</span>}
      </div>
            ...
            {section.items.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`mx-2 flex rounded-lg text-sm transition-colors ${effectiveCollapsed ? "justify-center px-2 py-3" : "items-center gap-3 px-4 py-2"} ${isActive
                ...
                >
                  ...
                  {!effectiveCollapsed && badge(item.badge)}
                </Link>
              );
            })}
          </div>
      ...
      <div className="px-3 pb-2">
        <button
          ...
          className={`flex w-full items-center rounded-xl border border-neutral-200 text-xs font-medium text-neutral-500 transition hover:bg-neutral-100 hover:text-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:hover:bg-neutral-800 dark:hover:text-neutral-100 ${effectiveCollapsed ? "justify-center px-2 py-2.5" : "gap-2 px-3 py-2.5"}`}
          title={isMobile ? "Close navigation" : effectiveCollapsed ? "Expand sidebar" : "Collapse sidebar"}
        >
          ...
          {!effectiveCollapsed && <span>{isMobile ? "Close menu" : effectiveCollapsed ? "Expand sidebar" : "Collapse sidebar"}</span>}
        </button>
      </div>

      {/* User profile */}
      <div ref={menuRef} className="relative shrink-0 border-t border-neutral-200 p-3 dark:border-neutral-800">
        <button
          onClick={() => setShowUserMenu((v) => !v)}
          title={effectiveCollapsed ? (mounted ? (user?.full_name ?? user?.identity ?? "User") : "User") : undefined}
          className={`flex w-full rounded-lg p-2 transition-colors hover:bg-neutral-100 dark:hover:bg-neutral-800 ${effectiveCollapsed ? "justify-center" : "items-center gap-3"}`}
        >
        ...
        {showUserMenu && (
          <div className={`z-50 rounded-xl border border-neutral-200 bg-white py-1 shadow-lg dark:border-neutral-700 dark:bg-neutral-800 ${effectiveCollapsed ? "absolute bottom-0 left-full ml-2 w-48" : "absolute bottom-full left-3 right-3 mb-1"}`}>
